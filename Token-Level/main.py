...
def train(
    ...
    if mode in ['adapter', 'memory', 'memory-ffn']:
        ...
        adapter_cfg = AdapterConfig(reduction_factor = 1024 // ffn_module_size,
                                    non_linearity='relu',
                                    mh_adapter=False,
                                    output_adapter=True,
                                    is_parallel=True)
    ...
    dir = os.path.join(os.path.abspath(os.getcwd()),
                       'saved_model', task + '_' + mode + '_' + str(memory_num))
    ...
    '''
        since there are public labels of test set on 
        both 3 NER datasets (MSRA, CoNLL2003, CoNLL2004), we just
        use trainer.test(...) to directly calculate evaluated metrics and do not
